...
def test_compute_bill_slots_emits_dense_zero_slots_when_no_samples():
    ...
    history = GridPowerHistory(samples=())

    out = compute_bill_slots(history, ps, t0, t0 + timedelta(hours=1))
...
def test_compute_bill_slots_imports_priced_with_buy():
    ...
    history = GridPowerHistory(samples=tuple(_samples_every_minute(t0, 15, power_kw=4.0)))

    out = compute_bill_slots(history, ps, t0, t0 + timedelta(minutes=15))
...
def test_first_run_with_empty_state_zeroes_carry_and_previous_month():
    ...
    result = build_monthly_bill_result(
        grid_history=GridPowerHistory(samples=()),
        price_series=ps,
        stored_state=None,
        local_tz=LOCAL_TZ,
        now=now,
...
def test_day_rollover_folds_yday_into_carry():
    ...
    history = GridPowerHistory(samples=tuple(samples))
